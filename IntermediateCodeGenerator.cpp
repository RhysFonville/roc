...
ASMVal IntermediateCodeGenerator::literal_expression(const std::shared_ptr<LiteralExpression>& expr) {
	if (expr->value.type == TokenType::TRUE) {
		...
		auto reg = std::make_shared<ASMValRegister>(str->held_type, occupy_next_reg());
		insert_command(IRCommand{IRCommand{IRCommandType::MOVE, {reg, str}}});
...
ASMVal IntermediateCodeGenerator::unary_expression(const std::shared_ptr<UnaryExpression>& expr) {
	...
	if (expr->op.type == TokenType::NOT) {
		insert_command(IRCommand{IRCommandType::XOR, {
		...
		insert_command(IRCommand{IRCommandType::NEG, {
			std::make_shared<ASMValRegister>(rhs->held_type, reg),
			rhs,
			std::nullopt
		}});
		...
		auto deref_reg{std::make_shared<ASMValRegister>(
			expr->type.get_type(),
			reg
		)};
		auto deref_rhs{std::make_shared<ASMValRegister>(*std::dynamic_pointer_cast<ASMValRegister>(rhs))};
		deref_rhs->dereferenced = true;
		insert_command(IRCommand{IRCommandType::MOVE, {
			deref_reg, deref_rhs, std::nullopt
		}});
	...
	unoccupy_if_reg(rhs);
	return std::make_shared<ASMValRegister>(expr->type.get_type(), reg);
}
	...
	switch (expr->op.type) {
		...
		case TokenType::SLASH: {
			...
			if (auto lhs_reg{std::dynamic_pointer_cast<ASMValRegister>(lhs)}) {
				...
				insert_command(IRCommand{type, {reg, lhs, rhs}});
				unoccupy_if_reg(lhs);
				unoccupy_if_reg(rhs);
				return reg;
			}
		...
		case TokenType::EQUAL: {
			if (auto id{std::dynamic_pointer_cast<IdentifierExpression>(expr->sides.first)}; id != nullptr) {
				commands.erase(commands.begin()+lhs_cmd_insert_spot-1);
				commands_insert--;

				unoccupy_if_reg(lhs);
				auto var{std::ranges::find_if(stacks.top().vars, [&](auto v){return v.name==id->identifier.value;})};
				lhs = std::make_shared<ASMValNonRegister>(var->type, var->name, true);
			}

			//auto mid{std::make_shared<ASMValRegister>(rhs->held_type, occupy_next_reg())};
			insert_command(IRCommand{IRCommandType::SET_VAR, {
				lhs,
				//mid,
				std::nullopt,
				rhs
...
void IntermediateCodeGenerator::variable_declaration_statement(const std::shared_ptr<VariableDeclarationStatement>& stmt) {
	auto src_reg{std::make_shared<ASMValRegister>(stmt->type->type.get_type(), occupy_next_reg())};
	insert_command(IRCommand{IRCommandType::MOVE, {
		src_reg,
	...
	insert_command(IRCommand{IRCommandType::DECL_VAR, {
		std::make_shared<ASMValNonRegister>(stmt->type->type.get_type(), stmt->identifier->identifier.value, true),
		src_reg
	}});
...
void IntermediateCodeGenerator::function_declaration_statement(const std::shared_ptr<FunctionDeclarationStatement>& stmt) {
	...
	if (stmt->identifier->identifier.value == "main") {
		...
		name = mangle_function(stmt);
	}
	funcs.insert({stmt->identifier->identifier.value, name});

	push_insert_spot(0);

	insert_command(IRCommand{IRCommandType::FUNC, {
		std::make_shared<ASMValNonRegister>(stmt->return_type->type.get_type(), name, true),
		std::nullopt,
		std::nullopt
	}});
	/*
